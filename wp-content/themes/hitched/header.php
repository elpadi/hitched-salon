...
<head>
	...
	<script>
		document.documentElement.className = [
			((('transform' in document.documentElement.style) || ('msTransform' in document.documentElement.style) || ('webkitTransform' in document.documentElement.style)) ? 'has' : 'no') + '-transform',
			(navigator.userAgent.indexOf('MSIE') > 0 ? 'is' : 'no') + '-ie',
			'js'
		].join(' ');
	</script>
	<?php wp_head(); ?>
